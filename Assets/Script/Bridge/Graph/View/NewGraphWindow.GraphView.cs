...
namespace YBFramework.MyEditor
{
    ...
    {
        private readonly Dictionary<string, NewCustomGraphView> m_DrawnGraphs = new();
        ...
        private NewCustomGraphView m_SelectedGraphView;
        ...
        private void ChangeMainGraphView(string graphAssetPath)
        {
            ...
            {
                if (!m_DrawnGraphs.TryGetValue(graphAssetPath, out NewCustomGraphView graphView))
                {
                    GraphAsset graphAsset = AssetDatabase.LoadAssetAtPath<GraphAsset>(graphAssetPath);
                    if (graphAsset != null)
                    {
                        graphView = new NewCustomGraphView(graphAsset, s_MainWindow.GetSearchView(graphAsset.GetGraphType()));
        ...
        public void CreateNodeInMainGraphView(Type nodeType, string nodeName, Vector2 nodePosition)
        {
            ...
            if (Activator.CreateInstance(nodeType) is BaseNode baseNode)
            {
                ...
                baseNode.ID = m_SelectedGraphView.m_BindGraphAsset.AllocateID();
